...
2. 读取 [视觉基础](references/foundations.md)、[布局和栅格](references/layout-and-grid.md) 与 [组件模式](references/component-patterns.md)；组件模式的通用规则必读，具体章节按页面实际使用的组件选读。
3. 根据任务按需读取：
...
5. 维护本 Skill、核对来源、补充本地规范未覆盖的组件，或任务明确要求 React/设计源对照时读取 [来源说明](references/provenance.md)。
...
- 只有任务明确要求 React 时才改变交付技术。
- HTML 与 React 原型的组件结构、尺寸、样式、适用状态和交互均必须遵循 [组件模式](references/component-patterns.md) 及其来源规范，按本地 tokens 与项目约束落实；生成界面保持品牌中立。
...
4. 按页面实际使用的组件逐项核对结构、尺寸、tokens、适用状态和交互是否符合组件规范；同时检查标题层级、空状态、错误状态和反馈。
