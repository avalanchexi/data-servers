...
4. **已确认的参考优先级：**HiMarket 是主要产品交互参考；Higress 只补充注册来源、路由匹配、策略作用域和 Schema 校验等网关配置细节；审批、发布差异、回滚、异常闭环和管理审计以 Data servers 已确认的领域规则为准。首版原型保持轻量，不展开厂商或基础设施深层参数。
...
| 审计日志 | 两个仓库都没有可核验的管理审计页面；Higress 的请求／响应日志插件记录的是流量内容，不是管理操作（来源：`/mnt/d/cursor/higress/plugins/release/console/log-request-response/spec.yaml:22-97`） | 按已确认范围提供不可修改简表：操作者、操作事项、关联对象、前后版本摘要、审批／发布编号、结果、时间；基础筛选与详情 | 请求／响应正文、访问日志全文检索、审计策略编辑、日志删除或修改 |
...
- 页面采用简要统计、只读列表和轻量详情。顶部统计近 24 小时操作数、失败数、高风险操作数和进行中数量。
- 列表展示时间、操作者、操作事项、关联对象、执行结果和关联编号；默认近 24 小时，只提供操作事项、执行结果和关键字筛选。
- 记录范围只覆盖路由变更、策略变更、配置发布、紧急停流和数据应用策略自动分配；执行结果为成功、失败、进行中。
- 详情展示基础信息、简要变更摘要、审批或发布关联和失败原因；敏感配置只显示引用变化，不显示秘密值。
- 所有记录不可修改或删除；不提供登录日志、基础设施操作、通用安全审计、全文检索、导出或审计策略配置。
...
- 本轮可核验的核心对象只提供注册来源、代理和插件运行配置；这些对象中没有 Data servers 所需的数据应用、平台审批、运行／开发版本、配置发布单、回滚发布、异常处置和管理审计页面模型。（来源：`/mnt/d/cursor/higress/api/networking/v1/mcp_bridge.proto:47-99`；`/mnt/d/cursor/higress/api/extensions/v1alpha1/wasmplugin.proto:50-127`）
- 请求／响应日志配置面向流量内容记录，不等于“谁修改了什么配置”的管理审计。（来源：`/mnt/d/cursor/higress/plugins/release/console/log-request-response/spec.yaml:22-97`）
